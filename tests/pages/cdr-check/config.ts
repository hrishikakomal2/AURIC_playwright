...
// loads the root .env) so this folder's own START_DATE/END_DATE never get confused with the Agent
// Performance suite's APR_* names or any other standard-report suite's own .env — see ./.env.
// Login credentials (TEST_BASE_URL/TEST_EMAIL/TEST_PASSWORD) deliberately are NOT duplicated here:
// they're read from the root .env, already loaded globally by playwright.config.ts before this
// runs — same account every other suite uses.
dotenv.config({ path: path.resolve(__dirname, '.env') });

export interface CdrCheckConfig {
...
export function loadCdrCheckConfig(): CdrCheckConfig {
